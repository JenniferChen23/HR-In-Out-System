...
<template>
    ...
    <template v-slot:item="{ item }">
      <tr>
        ...
        <template v-for="header in dynamicHeaders" :key="header.value">
          <td>
            <StatusCard
              :content="item[header.value]?.status || '-'"
              @click="() => openDetail(item, header.value)"
            />
          </td>
        </template>
      </tr>
  ...
  <v-dialog v-model="showDetail" persistent width="600">
    <v-card>
...
export default {
  ...
  computed: {
    resolvedHeaders() {
      const finalHeaders = Array.isArray(this.headers)
        ? this.headers
        : this.headers.value;
    ...
    dynamicHeaders() {
      return this.resolvedHeaders.filter(
        (header) => header.value !== "employeeId" && header.value !== "name"
      );
    },
  },
};
